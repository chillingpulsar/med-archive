...
const baseSchema = {
	type: z
		...
		.default('outpatient')
		.refine((value) => value === 'outpatient' || value === 'inpatient', {
...
export const addRecordSchema = z.object(baseSchema).extend({
	...
	billing_details: z
		.object({
			...
			payment_mode: z
				.string()
				.min(1, 'Must select a payment mode')
				.refine(
					(value) =>
						value === 'cash' || value === 'card' || value === 'cheque' || value === 'online',
					{
